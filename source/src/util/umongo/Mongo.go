...
func NewMongoDbUtilUseEnv(collectionName string) *MongoDbUtil {
	return &MongoDbUtil{
		Srv:            os.Getenv(config.ENV_MONGO_URI),
		DbName:         os.Getenv(config.ENV_MONGO_DB),
		CollectionName: collectionName,
		Ctx:            context.Background(),
	}
}
...
func (umongo *MongoDbUtil) connect() (newClient *mongo.Client, err error) {
	if umongo.useSentry {
		span := sentry.StartSpan(umongo.Ctx, "MongoDbUtil.connect")
		defer span.Finish()
	}
...
func (umongo *MongoDbUtil) BaseUpdateOne(filter, update bson.M) {
	...
	col := client.Database(umongo.DbName).Collection(umongo.CollectionName)
	...
	if !isUpdate {
		if paramAsReflect.Kind() == reflect.Map {
			...
			if idField.IsValid() && len(idField.String()) == 0 {
				idField.SetString(uid.GenerateID())
			}
		}
...
func (o MongoDbUtil) BaseFindOneMap(filter bson.M) (result interface{}, err error) {
	client, err := o.connect()
	var ress bson.D
	if err != nil {
		return
	}
	defer o.Disconnect(client)
	...
	if res.Err() != nil {
		...
		log.Println(err, o.CollectionName, string(filterAsJson))
...
func (umongo *MongoDbUtil) HardDelete(filter bson.M) (err error) {
	client, err := umongo.connect()
	if err != nil {
		return
	}
	defer umongo.Disconnect(client)
	col := client.Database(umongo.DbName).Collection(umongo.CollectionName)
	res, err := col.DeleteOne(umongo.Ctx, filter)
	if err != nil {
		log.Println(err)
		return
	}
	...
	if res.DeletedCount < 1 {
		err = errors.New("failed delete document")
		return
	}
	return
}
